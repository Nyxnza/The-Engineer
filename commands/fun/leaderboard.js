...
async function buildLeaderboard(type, guild) {
  ...
  if (type === 'alltime') {
    const raw = await db.getAllUsers();
    data = raw.map(u => ({ id: u.userId, count: u.xp })).slice(0, 10);
    title = '🏆 All Time XP Leaderboard';
  } else {
    data = (await db.getMessageCounts(type)).slice(0, 10);
    title = {
  ...
  const lines = data.map((entry, i) => {
    ...
    const name = member ? member.displayName : `<@${entry.id}>`;
